...
- Prisma + MySQL, custom JWT auth (jose), bcryptjs
...
- Custom JWT auth via `jose` library (HS256, 24h expiry) — **not** NextAuth v5 despite `next-auth` being in `package.json`.
- `auth.ts` re-exports `getSession` from `@/lib/auth/session`. `proxy.ts` middleware protects `/admin/*` by checking cookie existence.
- Login: `/admin/login` (client form at `app/admin/login/form.tsx`).
- Protected layout: `app/admin/(protected)/layout.tsx` redirects if not authenticated.
- `lib/auth/require-admin.ts` verifies session role in server components.
- `types/next-auth.d.ts` augments NextAuth types (legacy; actual JWT payload defined in `lib/auth/session.ts`).
...
- **`next.config.ts`** — `serverActions.bodySizeLimit: "25mb"`, `serverExternalPackages: ["@prisma/client"]`, remote images from `*.public.blob.vercel-storage.com`.
- **Uploads** — `lib/storage/local-storage.ts`: saves to `public/uploads/profile/` (images) or `public/uploads/documents/` (PDFs). MIME-restricted (JPEG/PNG/WebP/PDF), UUID filenames. Auto-detects `BLOB_READ_WRITE_TOKEN` to upload to Vercel Blob instead if set. Never hand-edit `public/uploads/`.
- **Admin panel** — `app/admin/(protected)/` has CRUD for profile, projects, skills, experiences, certificates, about. Each section has server actions in `actions.ts` files.
- **Light/dark mode** — Cookie + localStorage via `lib/theme-provider.tsx` (client component). Server reads `theme` cookie in `app/layout.tsx` and applies `.light` class on `<html>`.
