...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      const halfFloatTexType: number | undefined = isWebGL2
        ? (glContext as WebGL2RenderingContext).HALF_FLOAT
        : (halfFloat && (halfFloat as unknown as Record<string, number>).HALF_FLOAT_OES) ?? undefined;
    ...
    function getSupportedFormat(
      ...
      if (!supportRenderTextureFormat(gl, internalFormat, format, type)) {
        switch (internalFormat) {
          case 0x822b: // R16F
            return getSupportedFormat(gl, 0x822d, 0x8227, type); // RG16F → RG
          case 0x822d: // RG16F
            return getSupportedFormat(gl, 0x8814, 0x1908, type); // RGBA16F → RGBA
          default:
            return null;
        }
      }
    ...
    let dye: ReturnType<typeof createDoubleFBO>, velocity: ReturnType<typeof createDoubleFBO>;
    let divergence: ReturnType<typeof createFBO>, curl: ReturnType<typeof createFBO>, pressure: ReturnType<typeof createDoubleFBO>;
    ...
    function getUniforms(program: WebGLProgram): Record<string, WebGLUniformLocation | null> {
      ...
      for (let i = 0; i < uniformCount; i++) {
        const uniformName = (gl.getActiveUniform(program, i) as WebGLActiveInfo).name;
    ...
    function initFramebuffers() {
      const simRes = getResolution(config.SIM_RESOLUTION as number);
      const dyeRes = getResolution(config.DYE_RESOLUTION as number);
    ...
    function updateColors(dt: number) {
      colorUpdateTimer += dt * (config.COLOR_UPDATE_SPEED as number);
    ...
    function step(dt: number) {
      ...
      gl.uniform1f(vorticityProgram.uniforms["curl"], config.CURL as number);
      ...
      gl.uniform1f(clearProgram.uniforms["value"], config.PRESSURE as number);
      blit(pressure.write);
      pressure.swap();
      ...
      for (let i = 0; i < (config.PRESSURE_ITERATIONS as number); i++) {
      ...
      gl.uniform1f(advectionProgram.uniforms["dissipation"], config.VELOCITY_DISSIPATION as number);
      blit(velocity.write);
      velocity.swap();
      ...
      gl.uniform1f(advectionProgram.uniforms["dissipation"], config.DENSITY_DISSIPATION as number);
      blit(dye.write);
      dye.swap();
    }

    function render(target: { width: number; height: number; fbo: WebGLFramebuffer | null } | null) {
    ...
    function splatPointer(pointer: Pointer) {
      const dx = pointer.deltaX * (config.SPLAT_FORCE as number);
      const dy = pointer.deltaY * (config.SPLAT_FORCE as number);
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: [number, number, number]) {
      ...
      gl.uniform1f(splatProgram.uniforms["radius"], correctRadius((config.SPLAT_RADIUS as number) / 100.0));
      blit(velocity.write);
      velocity.swap();
